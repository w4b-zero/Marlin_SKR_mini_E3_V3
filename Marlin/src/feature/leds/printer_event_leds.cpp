...
PrinterEventLEDs printerEventLEDs;
  ...
  inline uint8_t pel_intensity_vb(const celsius_t start, const celsius_t current, const celsius_t target, int color_v = 0, int color_b = 255) {
    if (start == target) return color_b;
    return (uint8_t)map(constrain(current, start, target), start, target, color_v, color_b);
  }
...
//new(orange > red) old(violett > red)
#if HAS_TEMP_HOTEND
...
//  }
  void PrinterEventLEDs::onHotendHeating(const celsius_t start, const celsius_t current, const celsius_t target) {
     const uint8_t green = pel_intensity_vb(start, current, target, 0, 80);
    if (green != old_intensity) {
      old_intensity = green;
      pel_set_rgb(255, 80 - green, 0);
    }
  }
#endif
...
//  }
  void PrinterEventLEDs::onBedHeating(const celsius_t start, const celsius_t current, const celsius_t target) {
    const uint8_t red = pel_intensity(start, current, target);
    const uint8_t green = pel_intensity_vb(start, current, target, 0, 175);
    if (red != old_intensity) {
      old_intensity = red;
      pel_set_rgb(red, 255 - green, 0);
    }
  }
